    ...
    def start(self) -> int:
        ...
        while self.grid.canMove() and not self.over:
            ...
            if turn == PLAYER_TURN:
                ...
                if not(self.undoGrid):
                	self.undoGrid = self.grid.clone()#for initalization

                # If move is valid, attempt to move the grid
                if move != None and 0 <= move < 4:
                    if self.grid.canMove([move]):
                        self.undoGrid = self.grid.clone()
                        self.consecutiveResets=0
                        self.grid.move(move)
                        ...
                        self.over = True
                elif move != None and move == 4 and self.consecutiveResets < maxConsecutiveResets:
                	self.grid = self.undoGrid
                	self.consecutiveResets+=1
                elif self.consecutiveResets >= maxConsecutiveResets:
                	print("Max Consecutive Resets Reached")
                	self.over = True
                else:
